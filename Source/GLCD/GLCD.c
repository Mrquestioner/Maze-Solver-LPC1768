...
void PutChar2(uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor, uint16_t width, uint16_t height) {
    uint16_t i, j, x, y;
    uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer, ASCI);  /* Get character bitmap */
    
    // Compute scaling factors while ensuring they are at least 1 (to avoid division errors)
    uint16_t scaleX = (width + 7) / 8;   // Ensure proper rounding for width
    uint16_t scaleY = (height + 15) / 16; // Ensure proper rounding for height
    
    for (i = 0; i < 16; i++) { // Original character height is 16
        tmp_char = buffer[i];
        for (j = 0; j < 8; j++) { // Original character width is 8
            uint16_t pixelColor = ((tmp_char >> (7 - j)) & 0x01) ? charColor : bkColor;
            
            // Scale pixel into multiple pixels based on computed scaling
            for (y = 0; y < scaleY; y++) { // Scale vertically
                for (x = 0; x < scaleX; x++) { // Scale horizontally
...
void DisplayWelcomeMessage() {
    ...
    // Left-aligned text wrapping
    uint16_t textX = 10;  // Left margin
    char buffer[50];  // Store each word
    uint8_t j = 0;
    uint8_t lineLen = 0;
    ...
    for (uint16_t i = 0; message[i] != '\0'; i++) {
        if (message[i] == ' ' || message[i] == '\0') {
            ...
            lineLen += j + 1;  // Add word length + space
            j = 0;  // Reset buffer index
        } else {
            buffer[j++] = message[i];  // Store character
        }
    }
}
